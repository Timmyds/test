...
var delAuthority=inAuthority('wm-usc-group-area-del',funAuthority);
var editAuthority=inAuthority('wm-usc-group-area-edit',funAuthority);
var supplierAr=new Array();
var shopAr=new Array();
$(function() {
	if(funAuthority){
...
function saveGroup(){
	...
	if(text=="保存"){
	    ...
	    $('#supplier-select :selected').each(function(i, selected) {
		 ...
		 $('#shop-select :selected').each(function(i, selected) {
		 		authority +="3_"+$(selected).val()+",";
		 	  });
...
function supplierloadauthority(){
	...
	*/
		...
		initialShopList(shopContent,shopAr);
}
function shoploadauthority(){
	//var groupById=$("#add_groupId").val();
	var shopContent=$("#shopContent").val();
	var suppilerContent=$("#supplierContent").val();
	/*var supplierAr=new Array();
...
//管辖网点
function   initialShopList(content,dataAuthority){

	$.get("../../sysGroup/sysGroupfindShop.do",{content:''}, function(result) {
		if (result.status == 200) {
			$("#shop-select option").remove();
			var areaList=result.data;
			if(areaList){
				 $.each(areaList,function(index,data){ 
					 if( $.inArray(data.id, dataAuthority )>-1){
						 if(data.shopName){
						 $("#shop-select").append('<option value="'+data.id+'" data-section="'+data.contactProvince+'/'+data.contactCity+'" selected="selected">'+data.shopName+'</option>');
						 }
					 }else{
						 if(data.shopName){
						 $("#shop-select").append('<option value="'+data.id+'" data-section="'+data.contactProvince+'/'+data.contactCity+'">'+data.shopName+'</option>');
						 }
					 }
				 });
			}
			$("#shop-select").treeMultiselect({enableSelectAll: true, sortable: true });				
		}else {
			if(result.msg){
				$.messager.alert({
		    	      title:'提示',
